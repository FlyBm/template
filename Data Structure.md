...
#define rson(x) s[x].rc
  ...
  void insert(int &rt, int l, int r, int idx, int num) {
    ...
    if (idx <= mid)
      insert(lson(rt), l, mid, idx, num);
    else
      insert(rson(rt), mid + 1, r, idx, num);
  }

  int query(int rt, int l, int r) {
    if (l == r) return l;
    int mid = (l + r) >> 1;
    return s[s[rt].lc].num ? query(s[rt].lc, l, mid)
                           : query(s[rt].rc, mid + 1, r);
  }
  ...
  void split(int x, int &y, int k, bool flag) {
    ...
    if (flag) {
      ...
      if (num < k)
        split(s[x].rc, s[y].rc, k - num, flag);
      else
        swap(s[x].rc, s[y].rc);
      ...
      if (num < k)
        split(s[x].lc, s[y].lc, k - num, flag);
      else
        swap(s[x].lc, s[y].lc);
...
#define rson node << 1 | 1
  ...
  function<pair<int, int>(pair<int, int>, int)> merge =
      [&](pair<int, int> point, int c) -> pair<int, int> {
  ...
  void change(int node, int l, int r, int id, int val) {
    ...
    int mid = l + r >> 1;
    if (id <= mid)
      change(lson, l, mid, id, val);
    else
      change(rson, mid + 1, r, id, val);
    pushup(node);
  }
...
void build(int &p, int &q, int l, int r) {
  ...
  add(p, ls[p], 0), add(ls[q], q, 0);
  build(rs[p], rs[q], mid + 1, r);
  add(p, rs[p], 0), add(rs[q], q, 0);
}
...
void change(int node, int l, int r, int L, int R, int rt, int flag) {
  ...
  int mid = l + r >> 1;
  if (R <= mid)
    change(ls[node], l, mid, L, R, rt, flag);
  else if (L > mid)
    change(rs[node], mid + 1, r, L, R, rt, flag);
  else {
...
#define rev(x) fhq[x].rev
  ...
  inline void update(int now) { siz(now) = siz(l(now)) + siz(r(now)) + 1; }
...
#define r(x) fhq[x].r
  ...
  inline void getnum(int rank, int &val) {
    int now = root;
    while (now) {
      if (fhq[l(now)].siz + 1 == rank)
        break;
      else if (fhq[l(now)].siz >= rank)
        now = l(now);
      else {
...
#define rson node << 1 | 1
  ...
  void
  pushup(int node, int l, int r) {
    for (int i = 0; i <= 30; ++i) {
      s[node].num[i] = s[lson].num[i] + s[rson].num[i];
    }

    s[node].minx = s[lson].minx, s[node].cnt = s[lson].cnt,
    s[node].seminx = s[lson].seminx;
  ...
  __attribute__((optimize("O3"), target("avx"))) void spread(int node, int l,
                                                             int r) {
  ...
  __attribute__((optimize("O3"), target("avx"))) void build(int node, int l,
                                                            int r) {
    if (l == r) {
  ...
  __attribute__((optimize("O3"), target("avx"))) void change(int node, int l,
                                                             int r, int L,
                                                             int R, int x) {
    if (L == l and R == r) {
      if (x <= s[node].minx)
    ...
    if (R <= mid)
      ...
      change(lson, l, mid, L, mid, x), change(rson, mid + 1, r, mid + 1, R, x);
    pushup(node, l, r);
  }

  __attribute__((optimize("O3"), target("avx"))) int query(int node, int l,
                                                           int r, int L, int R,
                                                           int maxbit) {
    if (L <= l and R >= r) {
  ...
  __attribute__((optimize("O3"), target("avx"))) int querysum(int node, int l,
                                                              int r, int L,
                                                              int R) {
    if (L <= l and R >= r) {
      return s[node].sum;
...
// codeforces 282E
// 给了一个长度为 n(1 ≤ n ≤ 1e5)
// 的数组，求一个不相交的前缀和后缀，使得这个前缀和后缀中的所有数的异或值最大
...
struct star {
  ll l, r, _l, _r, val;
  bool operator<(const star &rhs) const { return val < rhs.val; }
};
...
#define rson(x) s[x].rc
  ...
  void insert(int &now, int pre, int l, int r, int idx, int val) {
    ...
    int mid = l + r >> 1;
    if (idx <= mid)
      insert(lson(now), lson(pre), l, mid, idx, val);
    else
      insert(rson(now), rson(pre), mid + 1, r, idx, val);
  }
...
int main() {
  ...
  for (int i = 1; i <= m; ++i) {
    if (s[i].type == 1) {
      totone = 0, tottwo = 0;
      for (int j = s[i].l - 1; j > 0; j -= lowbit(j))
        qone[++totone] = query.root[j];
      for (int j = s[i].r; j > 0; j -= lowbit(j))
        qtwo[++tottwo] = query.root[j];
...
int main() {
  ...
  sort(road + 1, road + 1 + m, [](node a, node b) { return a.dis > b.dis; });
    ...
    while (top[x] != top[y]) {
      if (dep[top[x]] >= dep[top[y]]) {
        if (flag) {
          ...
          now = {(now.first and star.first and now.second == star.second),
                 star.second};
        }
        x = f[top[x]];
      } else {
