...
int dep[N], cur[N], tot = -1, head[N], S, T, n, m, ss,
                    tt;  // ss tt 真正的源汇点 S T 超源超汇
...
int main() {
  ...
  for (int i = 1; i <= n; ++i) {
    if (in[i] > out[i])
      add_net_edge(S, i, in[i] - out[i]);
    else if (in[i] < out[i])
      add_net_edge(i, T, out[i] - in[i]);
  }
...
struct PushRelabelMinCostMaxFlow {
  ...
  PushRelabelMinCostMaxFlow(flowUnit FLOW_EPS, costUnit COST_INF,
                            costUnit COST_EPS)
...
ll val(ll x, ll y, ll z) { return sgn(x) + sgn(y) + sgn(z); }
...
int main() {
  ...
  for (int i = 1; i <= n; ++i) {
    for (int j = 1; j <= n; ++j) {
      mcmf.addEdge(i - 1, j + n - 1, 1,
                   val(p[j].x, p[j].y, p[j].z + 1ll * (i - 1) * p[j].v));
    }
  }
