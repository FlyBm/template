...
sort(node + 1,node + n + 1,cmpx);Blocking(1,n);printf("%.4f\n",minx);
}
```
...
int line_inter_seg(Point a , Point b , Point c , Point d) // ab直线cd线段
{
    ...
    l1 = a; l2 = b;
    double step = 1e5;
    l2 = l2 - l1; l2 = l2 * step , l2 = l2 + l1;
    l1 = l1 - b; l1 = l1 * step , l1 = l1 + b;
    if (seg_inter_seg(l1 , l2 , st , ed)) return 1;
    else return 0;
}
//直线与直线相交
...
Point* Cutline::intersec(line* L1, line* L2)
{
	...
	double y = k1 * (x - x1) + y1;
	if ((x - x1)*(x-x2) <0 && (y-y1)*(y-y2)<0 && (x-x3)*(x-x4)<0 && (y-y3)*(y-y4)<0)//判断是否在范围中
	{
		//构建新的交点
		//cout << "范围内" << endl;
		Point* temp;
...
inline double RC()
{
    ...
    for (int i = 1 ; i <= n ; i ++)
    {
        ...
        {
            while(a % top + 1 != j && sgn(fabs(Cross (s[j] - s[i] , s[a] - s[i])) - fabs(Cross(s[j] - s[i] , s[a + 1] - s[i])) <= 0))
            {
                a = a % top + 1;
            }
            while(b % top + 1 != i && sgn(fabs(Cross (s[b] - s[i] , s[j] - s[i])) - fabs(Cross(s[b + 1] - s[i] , s[j] - s[i])) <= 0))
            {
                b = b % top + 1;
            }
...
struct T3dhull
{
    ...
    bool same(int s , int e)
    {
        ...
        return (fabs(volume(a,b,c,ply[tri[e].a])) < eps && fabs(volume(a,b,c,ply[tri[e].b])) < eps && fabs(volume(a,b,c,ply[tri[e].c])) < eps);
    }
    void construct()
    {
        tri_cnt = 0;
