...
inline bool operator<(const P& a, const P& b) {
  return sgn(a.x - b.x) < 0 or (sgn(a.x - b.x) == 0 && sgn(a.y - b.y) < 0);
}
...
// 逆时针旋转 r 弧度
P rotation(const P& p, const LD& r) {
...
int s_cross(const L& a, const L& b, P& p) {
  ...
  if ((d1 ^ d2) == -2 && (d3 ^ d4) == -2) {
    p = l_intersection(a, b);
    return 1;
  }
  if (!d1 && p_on_seg(b.s, a)) {
    p = b.s;
    return 2;
  }
  if (!d2 && p_on_seg(b.t, a)) {
    p = b.t;
    return 2;
  }
  if (!d3 && p_on_seg(a.s, b)) {
    p = a.s;
    return 2;
  }
  if (!d4 && p_on_seg(a.t, b)) {
    p = a.t;
    return 2;
  }
  return 0;
...
LD c_tri_area(P a, P b, P center, LD r) {
  ...
  if (ina && inb) {
    ...
    if (ina ^ inb) {
      ...
      if (ina)
        return sector_area(b, cr, r) + fabs(cross(a, cr)) / 2;
      else
        return sector_area(a, cr, r) + fabs(cross(b, cr)) / 2;
    } else {
...
struct CV {
  ...
  int type;
  CV() {}
  CV(LD yl, LD yr, LD ym, C c, int t) : yl(yl), yr(yr), ym(ym), type(t), o(c) {}
};
...
LD circle_union(const vector<C>& cs) {
  ...
  xs.erase(
      unique(xs.begin(), xs.end(), [](LD x, LD y) { return sgn(x - y) == 0; }),
      xs.end());
  LD ans = 0;
  FOR(i, 0, (int)xs.size() - 1) {
    ...
    FOR(k, 0, n) {
      ...
      if (sgn(c.p.x - c.r - xl) <= 0 && sgn(c.p.x + c.r - xr) >= 0) {
        ...
        intv.push_back(t.first);
        intv.push_back(t.second);
      }
    }
    ...
    FOR(i, 0, intv.size()) {
      if (cc > 0) {
        ans += (intv[i].yl - intv[i - 1].yl + intv[i].yr - intv[i - 1].yr) *
               (xr - xl) / 2;
...
inline int operator<(const P& a, const P& b) {
  return sgn(a.x - b.x) < 0 ||
         (sgn(a.x - b.x) == 0 &&
          (sgn(a.y - b.y) < 0 || (sgn(a.y - b.y) == 0 && sgn(a.z - b.z) < 0)));
}
bool operator==(const P& a, const P& b) {
...
istream& operator>>(istream& is, P& p) { return (is >> p.x >> p.y >> p.z); }
...
P rotation(const P& p, const LD& r, const P& n) {
  ...
  return P((x * x * (1 - c) + c) * p.x + (x * y * (1 - c) + z * s) * p.y +
               (x * z * (1 - c) - y * s) * p.z,
           (x * y * (1 - c) - z * s) * p.x + (y * y * (1 - c) + c) * p.y +
               (y * z * (1 - c) + x * s) * p.z,
           (x * z * (1 - c) + y * s) * p.x + (y * z * (1 - c) - x * s) * p.y +
               (z * z * (1 - c) + c) * p.z);
}
```
...
int p_on_plane(const F& f, const P& p) {
  return sgn(dot(norm(f), p - f.a)) == 0;
}
...
int s_intersect(const L& u, const L& v) {
  return p_on_plane(F(u.s, u.t, v.s), v.t) && opposite_side(u.s, u.t, v) &&
