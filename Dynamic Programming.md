...
void dfs(int node, int fa) {
  ...
  for (int to : v[node]) {
    ...
    if (dp[to][1] > dp[to][0] and not flag)
      res = min(res, dp[to][1] - dp[to][0]);
  }
  ...
  for (auto to : v[node]) {
    ...
    for (int j = siz[node]; j >= 0; --j) {
      for (int k = siz[to]; k >= 0; --k) {
        dp[node][j + k][0] =
            max(dp[node][j][0] + dp[to][k][0], dp[node][j + k][0]);
        if (k >= 1)
          dp[node][j + k][0] =
              max(dp[node][j + k][0], dp[node][j][0] + dp[to][k][1] + w[to]);
        if (k + j >= 1 and j >= 1)
          dp[node][j + k][1] =
              max(dp[node][j + k][1], dp[node][j][1] + dp[to][k][0] + w[to]);
        if (k + j >= 1 and j >= 1 and k >= 1)
          dp[node][j + k][1] =
              max(dp[node][j + k][1], dp[node][j][1] + dp[to][k][1] + w[to]);
      }
    }
    siz[node] += siz[to];
  }
